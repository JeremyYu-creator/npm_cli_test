      ...
      inquirer.prompt([
        ...
        child_process.exec('git clone ' + url, function (err) {
          if (err) {
            ...
            // __dirname 当前模块的目录名
            // shell你可以理解为一个cmd
            const pathTest = process.cwd()
            // 此处的文件是截取项目的名字，这里项目的名字是git上的名字，然后替换成你想要的名字
            // shell.mv(__dirname + '/es-template-test', __dirname + '/' + name)
            shell.mv(pathTest + '/es-template-test', pathTest + '/' + name)
            // console.log('当前的路径名称', __dirname + '/es-template-test', pathTest)
            ...
            // console.log(__dirname,'当前文件路径',fs)
